...
import { useGetWorkActivityStatesQuery } from '../../../redux/apiSlices/dashboardSlice';

const DoctotWorkActivitiesChart = () => {
  const currentYear = new Date().getFullYear();
  const years = Array.from({ length: 12 }, (_, i) => currentYear - 10 + i);
  const [selectedYear, setSelectedYear] = useState(currentYear);
  const { data: userState, isFetching, refetch } = useGetWorkActivityStatesQuery(selectedYear);
    ...
    <div className="bg-white p-6 rounded-xl">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">
          <span className="font-bold text-2xl">Work</span> Activities
        </h2>
...
export default DoctotWorkActivitiesChart;
